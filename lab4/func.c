			...
			int Ones, int iter, int X, int Y, int Z, int n)
{
	int i, j;
   	int index_X, index_Y;
	int max_size = X*Y*Z;
#pragma omp parallel for num_threads(30) firstprivate(arrayX, arrayY, seed, n)
   	for(i = 0; i < n; i++){
   		arrayX[i] += 1 + 5*rand2(seed, i);
   		arrayY[i] += -2 + 2*rand2(seed, i);
   	}
#pragma omp parallel for num_threads(30) private(index_X, index_Y, j) firstprivate(max_size, Y, Z, n, arrayX, arrayY, probability, array, objxy, index, Ones, iter)
...
void func2(double *weights, double *probability, int n)
{
	int i;
	double sumWeights=0;
#pragma omp parallel for num_threads(30) firstprivate(weights, probability, n)
	for(i = 0; i < n; i++)
   		weights[i] = weights[i] * exp(probability[i]);
#pragma omp parallel for num_threads(30) firstprivate(weights) reduction(+:sumWeights)
   	for(i = 0; i < n; i++)
   		sumWeights += weights[i];
#pragma omp parallel for num_threads(30) firstprivate(weights, sumWeights)
	for(i = 0; i < n; i++)
...
{
	int i, j;
#pragma omp parallel for num_threads(30) private(i, j) firstprivate(n, cfd, arrayX, arrayY, x_j, y_j, u)
	for(j = 0; j < n; j++){
...
#pragma omp parallel for num_threads(30) private(i, j) firstprivate(n, arrayX, arrayY, x_j, y_j, weights)
	for(i = 0; i < n; i++){
		arrayX[i] = x_j[i];
		arrayY[i] = y_j[i];
		weights[i] = 1/((double)(n));
